...
function buildReceiptData(row: PaymentRow): ReceiptPrintData {
  return {
...
/** Compact row: phone — date, amount, recorder, and payment type, plus a print button. */
    ...
    <View className="flex-row items-center justify-between border-b border-slate-100 px-4 py-3">
      <View className="flex-1">
        ...
        <Text variant="muted" className="mt-0.5 text-xs">
          {row.studentName} · {formatShortDate(row.occurredAt)} · {row.recordedBy}
...
export default function ReceptionistPaymentsScreen() {
  ...
  const fetchPage = useCallback(
    async (targetPage: number, opts: { silent?: boolean } = {}) => {
      ...
      try {
        ...
        // Same school-wide endpoint the admin Payments page uses — a
        // receptionist needs to find and reprint any payment, not just ones
        // for locally-cached students, so this is fetched straight from the
        // server (paginated) rather than read from the offline cache.
        const res = await api.get<PaymentsResponse>("/payments/admin", {
  ...
  function handlePrint(row: PaymentRow) {
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      <Fab accessibilityLabel="New payment" onPress={() => router.push("/receptionist/payments/new")} />
